...
12. **公開リポへフレームワーク固有名詞を書かない**（Issue #403 ブラックボックス化方針）：
    - 禁止語：`オールA` / `S軸宣言` / `S軸` / `PDCAS` / `品質4軸` / `卓越の戦略` / `ハイパワー・マーケティング` / `庇護者`
    - 代替表現：`全軸ベースライン以上` / `重点軸宣言` / `重点軸` / `改善サイクル` / `品質指標`
    - 適用範囲：docs / Issue / PR タイトル・本文 / コミットメッセージ
    - ID 体系（`M-`/`S-`/`SEC-`/`P-`）は一般的な命名規則として継続使用可
13. **親ドキュメント（リポ外管理）への直接リンクを docs に書かない**：
    - 禁止：`[母要件定義書](../../母要件定義書.md)` 等の相対パス参照
