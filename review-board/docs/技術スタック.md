...
> エンジニアの仕事」）、② 重点軸（セキュリティ・認可）の強化（RBAC / IDOR / 依存脆弱性スキャン）である。
...
│                                             │
│   ・REST API (/api/*)                        │
│   ・Spring Security による RBAC + 所有者/cohort 検証 (★重点軸)│
│   ・SPA 配信 (/)                             │
...
│  ・posts (成果物)       │
│  ・reviews             │
│  ・review_axis_comments│  ← 品質指標の観点別コメント
│  ・thanks (ありがとう)  │
│  ・evaluations (講師評価)│
│  ・audit_logs (★重点軸)   │
└────────────────────────┘
...
詳細は [機能一覧.md](./機能一覧.md)（後日作成）参照。本アプリは 重点軸（認可）が突出点のため、
**全エンドポイントで「ログイン必須 → ロール判定（RBAC）→ 所有者 / cohort 一致の検証」を必ず通す**
（要件定義書 §3-2、SEC-2/3/4）。不一致は 404 を返し IDOR を遮断する。
...
- Spring Web（REST API）
- **Spring Security**（★重点軸の中核）：RBAC（受講生 / 講師）・認可フィルタ・メソッドレベル認可（`@PreAuthorize`）
...
- **Spring Security + JWT（jjwt）** — sns-board / slack-board と同方針
  ...
  - **JWT in HttpOnly Cookie**（access 短命 + refresh DB rotation + reuse detection、SEC-7）
  - **DB rotation**: `refresh_tokens` テーブルに SHA-256 hash を保存、refresh ごとに使い捨て rotation
- **RBAC**：ロール（受講生 / 講師）をロールベースで一元判定（SEC-4）。最終評価・承認（`F-EVAL-01`）は講師ロール限定
- **IDOR 対策**：所有者 / cohort 一致をバックエンドで必ず検証、不一致は 404（SEC-2/3）
- **監査ログ**：`audit_logs` に「誰が・いつ・誰の資源に・何をしたか」を記録（重点基準）

### 2-5. 依存脆弱性スキャン（★重点軸・CI 常設）

- バックエンド: **OWASP dependency-check**（Gradle plugin）で High/Critical 0 件を維持
- フロントエンド: **`npm audit`** を CI に常設
- 要件定義書 §4-1 の 重点基準（High/Critical 0 件維持）に対応
...
- 画像は外部ストレージに保存し DB 肥大化を回避（P-10・SEC-8）
...
| **認可 (RBAC/IDOR)** | Spring Security `@PreAuthorize` + 所有者/cohort 検証 | Spring Boot 内蔵 | ➕ 追加（★重点軸の中核） |
| DB マイグレーション | Flyway | Spring Boot 内蔵 | ✅ 同一 |
| 依存脆弱性スキャン | OWASP dependency-check | Gradle plugin | ➕ 追加（★重点軸・CI 常設） |
...
- すべての依存は §2-5 の脆弱性スキャン対象。High/Critical は 0 件を維持（★重点軸）

---

## 8. M-14：Toolchain と実行ランタイムの整合（最重要）

> M-14。**「ビルド時に使う Java」と「本番 EC2 で動かす Java」を完全一致**させる。
...
- 共通の設計方針（リポジトリ外で管理） — M-14・SEC・P 各要件の原典
