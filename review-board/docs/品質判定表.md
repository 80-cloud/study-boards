...
| UX-2 破壊操作は確認 + 取り消し可能 | A | **#498 で ConfirmDialog + UndoBar を新設**し、投稿削除 / レビュー削除 / メンバー無効化に適用。バックエンドに `POST /api/posts/{id}/restore` `POST /api/reviews/{id}/restore` を追加し、`deletedAt` から 30 秒以内なら deletedAt=null に戻す（非正規化カウンタも巻き戻し）。所有者外・期限超過は 404（IDOR 遮断と同じ方針）。`UndoProvider` を App 直下に置き、ページ遷移をまたいでバーが生き残る。AuditAction に POST_RESTORED / REVIEW_RESTORED を追加。Integration test 3 件追加（owner_can_restore / other_cannot_restore / review owner+other）|
| UX-3 ヘルプ / FAQ ページ提供 | A | **#494 で `/help` を新設**（`HelpPage.jsx`・8 セクション：はじめて / 投稿 / レビュー / 多軸評価 / 招待 / 通知 / 削除 / 問い合わせ）。ネイティブ `<details>` で aria 対応＆ライブラリ追加なし。Shell に小フッターを追加し全認証ページから到達可能。`LoginPage` / `RegisterPage` / `LegalLayout` からもリンク。実機 Chrome で 8 セクション開閉確認済 |
