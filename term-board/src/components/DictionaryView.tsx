...
export function DictionaryView({ bookmarks }: Props) {
  ...
  return (
    <section className="flex flex-col gap-4">
      <div className="flex flex-col gap-3 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
        <div>
          ...
          <input
            ...
            className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-800 focus:border-sky-500 focus:outline-none focus:ring-2 focus:ring-sky-400"
          />
        </div>
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2">
            <label htmlFor="dict-cat" className="text-sm font-medium text-slate-600">分野</label>
            <select
              id="dict-cat"
              value={category}
              onChange={(e) => setCategory(e.target.value)}
              className="rounded-lg border border-slate-300 bg-white px-2 py-1.5 text-sm text-slate-800 focus:border-sky-500 focus:outline-none focus:ring-2 focus:ring-sky-400"
            >
      ...
      <p className="text-sm text-slate-600">{filtered.length} 件</p>

      <ul className="flex flex-col gap-2">
        {filtered.map((t) => {
          const open = openId === t.id;
          return (
            <li key={t.id} className="rounded-xl bg-white shadow-sm ring-1 ring-slate-200">
              <div className="flex items-center gap-2 p-3">
                ...
                >
                  <span className="truncate font-semibold text-slate-900">{t.term}</span>
                  {t.level && (
                    <span className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-medium ${levelClass[t.level] ?? "bg-slate-100 text-slate-700"}`}>
                      {t.level}
                    </span>
                  )}
                  <span className="shrink-0 text-xs text-slate-500">{t.category}</span>
                </button>
                <button
                  type="button"
              ...
              {open && (
                <div className="border-t border-slate-100 px-3 py-3 text-sm leading-relaxed">
                  {t.plainMeaning && (
                    <p className="text-slate-700"><span className="font-semibold text-sky-800">かんたんに：</span>{t.plainMeaning}</p>
                  )}
                  <p className="mt-1 text-slate-700"><span className="font-semibold text-slate-900">意味：</span>{t.meaning}</p>
                  {t.scene && (
                    <p className="mt-1 text-slate-600"><span className="font-semibold text-slate-800">現場では：</span>{t.scene}</p>
                  )}
                  <p className="mt-1 text-slate-600"><span className="font-semibold text-slate-800">面接での言い方：</span>{t.interview}</p>
                </div>
              )}
            </li>
          );
        ...
        {filtered.length === 0 && (
          <li className="text-center text-sm text-slate-600">該当する用語がありません。</li>
        )}
      </ul>
    </section>
  );
