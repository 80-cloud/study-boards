...
- **主要13モード実装・本番公開済み**（4択/暗記カード/用語辞典/学ぶ/面接練習/模擬面接/解説集/ダッシュボード/振り返り/自己PR/マイ問題/逆質問）。用語**218語**。オープンPRなし・main整合済み。
...
- **設計思想（重要）**：認証・サーバー・DBを**意図的に持たない**（過剰機能を持つと品質低下リスクが増える・要件定義書 §1-5）。
...
### ② 用語データの正確性レビュー＋出典確定（218語）✅ 実施済（#363。以後 #391で+15、#393で+12＋既存2件改良、#395でGit日常操作+15）
